...
export const serviceSchema = defineType({
  ...
  fields: [
    ...
    defineField({ name:'bestFor', title:'Best For', type:'text', rows:3, description:'Short explanation used on the Services page and service detail page.' }),
    defineField({ name:'availability', title:'Availability / Area', type:'text', rows:2, description:'Example: Remote across the UK or home visits in South East London and parts of Kent.' }),
    defineField({ name:'serviceAreaLabel', title:'Service Area Label', type:'string', description:'Short label chip, e.g. "Remote or home visit" or "Door-to-door transport".' }),
    defineField({ name:'ctaLabel', title:'Primary CTA Label', type:'string', description:'Example: "Book remote consultation" or "Ask about pet transport".' }),
    defineField({ name:'pricingNote', title:'Booking / Pricing Note', type:'text', rows:3, description:'Shown publicly under enquiry-based booking text. Keep it general if prices should stay private.' }),
    defineField({ name:'hidePublicPricing', title:'Hide Public Pricing', type:'boolean', initialValue:true, description:'Keep enabled if the client does not want prices visible on the website.' }),
