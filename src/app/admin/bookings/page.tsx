...
export default function AdminBookingsPage() {
  ...
  const handleGenerateLink = () => {
    ...
    const desc = `Soultails - ${formatServiceTypeLabel(selected.serviceType)} for ${selected.petName}`
  ...
  return (
    <div className="max-w-6xl">
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="input-field w-auto text-sm pr-8">
        ...
        <select value={modeFilter} onChange={(e) => setModeFilter(e.target.value)} className="input-field w-auto text-sm pr-8">
            ...
            <div className="divide-y divide-linen">
              {bookings.map((booking: any) => (
                <button
                  ...
                  className={`w-full text-left px-5 py-4 hover:bg-cream transition-colors ${selected?.id === booking.id ? 'bg-primary-light' : ''}`}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-body text-sm font-semibold text-brand-text">{booking.clientName}</span>
                        <span className="font-body text-xs text-brand-subtle">· {booking.petName} ({booking.petType})</span>
                      </div>
                      <div className="font-body text-xs text-brand-muted mt-0.5">
                        {formatServiceTypeLabel(booking.serviceType)} · {booking.serviceMode === 'HOME_VISIT' ? 'Home visit' : 'Remote'}
                      </div>
                    ...
                    <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                      <span className={`font-body text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_COLORS[booking.status] ?? 'bg-gray-100 text-gray-600'}`}>
                        {booking.status}
                      </span>
                      {booking.paymentStatus === 'PAYMENT_LINK_SENT' ? <span className="font-body text-xs text-blue-600 font-medium">PayPal sent</span> : null}
                      {booking.paymentStatus === 'PAID' ? <span className="font-body text-xs text-green-600 font-medium">Paid</span> : null}
                    </div>
                  </div>
                </button>
              ))}
        ...
        {selected ? (
          <div className="w-80 flex-shrink-0 bg-white rounded-xl border border-linen p-5 h-fit sticky top-0 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setSelected(null)} className="text-brand-subtle hover:text-brand-text">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="space-y-2 text-sm font-body">
              {[
                ...
                ['Service', formatServiceTypeLabel(selected.serviceType)],
                ...
                <div key={key} className="flex gap-2">
                  <span className="text-brand-subtle w-20 flex-shrink-0">{key}</span>
                  <span className="text-brand-text font-medium break-all">{value}</span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {selected.status === 'PENDING' ? (
                <button onClick={() => updateStatus(selected.id, 'CONFIRMED')} disabled={updating} className="btn-primary w-full justify-center text-sm py-2.5 disabled:opacity-60">
              ...
              {selected.status === 'CONFIRMED' ? (
                <button onClick={() => updateStatus(selected.id, 'COMPLETED')} disabled={updating} className="btn-outline w-full justify-center text-sm py-2.5 disabled:opacity-60">
                  Mark completed
                </button>
              ) : null}
            </div>
            ...
            {selected.paymentStatus !== 'PAID' ? (
              <div className="border-t border-linen pt-4 space-y-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={handleGenerateLink} disabled={!payAmount} className="btn-primary text-sm px-3 py-2 disabled:opacity-60">
                    <Send size={14} />
                  </button>
                </div>
