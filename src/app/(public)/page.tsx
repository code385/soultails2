...
import { SHARED_PAYMENT_MESSAGE } from '@/lib/serviceCatalog'
...
const features = [
  ...
  'Integrative and conventional medicine combined',
  'Feline behaviour expertise - ISFMAdvCerFB',
  'Home visits and remote consultations',
  'Senior and palliative care focus',
  ...
  const featuredPosts =
    sanityFeaturedPosts.length > 0
      ...
      : sanityAllPosts.length > 0
        ...
        : STATIC_POSTS.filter((post) => post.featured).slice(0, 3)
    ...
    <>
      <section className="relative min-h-screen flex items-center pt-20 overflow-hidden" style={{ background: 'var(--color-cream-dark)' }}>
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse at 80% 20%, #F5EDF9 0%, transparent 60%), radial-gradient(ellipse at 20% 80%, #FCEEF1 0%, transparent 60%)',
          }}
        />
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div className="max-w-xl">
              ...
              <div className="flex flex-wrap gap-3 mb-10">
                ...
                <Link href="/services" className="btn-outline px-7 py-3.5 text-base">
                  Our services
                </Link>
              </div>
              <div className="flex flex-wrap gap-2">
                {credentials.map((credential: string) => (
                  <span key={credential} className="font-body text-xs font-semibold px-3 py-1.5 bg-white border border-linen rounded-full text-brand-muted">
                    {credential}
                  </span>
                ))}
              </div>
              ...
              <div className="card p-8">
                ...
                <div className="space-y-3">
                  {features.map((feature) => (
                    <div key={feature} className="flex items-start gap-2.5">
                      ...
                      <span className="font-body text-sm text-brand-muted">{feature}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-6 pt-5 border-t border-linen flex items-center gap-2">
                  <div className="flex -space-x-1">
                    {[...Array(4)].map((_, index) => (
                      <div key={index} className="w-6 h-6 rounded-full border-2 border-white" style={{ background: 'var(--color-primary-light)' }} />
                    ))}
                  </div>
                  <div className="flex items-center gap-1">
                    {[...Array(5)].map((_, index) => (
                      <Star key={index} size={11} style={{ color: '#B784C4', fill: '#B784C4' }} />
                    ))}
                  </div>
      ...
      <section className="section-pad bg-white">
        <div className="container-site">
          <div className="text-center mb-14">
            ...
            <h2 className="font-heading text-3xl lg:text-4xl font-semibold text-brand-text mb-4">How booking works</h2>
            <div className="divider-brand mx-auto" />
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { step: '01', title: 'Choose your service', desc: "Browse the services and choose the option that best fits your pet's needs, or ask for help choosing." },
              { step: '02', title: 'Send your enquiry', desc: 'Share your pet\'s details, concern, preferred date and whether you need a remote consultation or home visit.' },
              { step: '03', title: 'Get confirmed care', desc: SHARED_PAYMENT_MESSAGE },
            ].map(({ step, title, desc }) => (
        ...
        <div className="container-site">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {featuredServices.map((service: any) => {
              ...
              return (
                <Link key={service.slug} href={`/services/${service.slug}`} className="card p-6 group block">
                  <div
                  ...
                  <h3 className="font-heading text-lg font-semibold text-brand-text mb-2 group-hover:text-primary transition-colors">{service.name}</h3>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-linen">
                    ...
                    <span
                      className="w-8 h-8 rounded-full border border-linen flex items-center justify-center transition-all duration-200 group-hover:border-primary"
      ...
      <section className="cta-gradient py-16">
        <div className="container-site text-center">
          ...
          <p className="font-body text-lg mb-8 max-w-xl mx-auto" style={{ color: 'rgba(255,255,255,0.8)' }}>
            Remote consultations available across the UK. Home visits in South East London and parts of Kent.
          </p>
          <Link
            href="/book"
            className="inline-flex items-center gap-2 px-8 py-4 bg-white font-body font-bold text-sm rounded-full hover:bg-cream transition-all duration-200 active:scale-[0.98]"
          ...
          <div className="container-site">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {featuredPosts.map((post: any) => {
                ...
                return (
                  <Link key={post._id ?? post.slug} href={`/blog/${slug}`} className="card overflow-hidden group block">
                    <div className="h-44 flex items-center justify-center overflow-hidden" style={{ background: 'linear-gradient(135deg,#FCEEF1,#F5EDF9)' }}>
                      {post.coverImage ? (
                        <img src={urlFor(post.coverImage).width(600).height(176).url()} alt={post.coverImage.alt ?? post.title} className="w-full h-full object-cover" />
                      ) : (
                        <span className="text-4xl">Pet care</span>
                      )}
                    </div>
                    <div className="p-5">
                      ...
                      <h3 className="font-heading text-lg font-semibold text-brand-text mb-2 group-hover:text-primary transition-colors line-clamp-2">
                        {post.title}
                      </h3>
                      <p className="font-body text-sm text-brand-muted line-clamp-2 mb-3">{post.excerpt}</p>
