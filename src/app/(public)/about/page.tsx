...
import { DOCTOR_PROFILE_IMAGE_FALLBACK } from "@/lib/serviceCatalog"
...
export default async function AboutPage() {
  ...
  const profileImageSrc = settings?.profileImage
    ? urlFor(settings.profileImage).width(500).height(500).fit("crop").url()
        ...
        <div className="container-site">
          <div className="max-w-3xl">
            ...
            <h1 className="font-heading text-4xl lg:text-5xl font-bold text-brand-text mb-6">
              Meet Dr. Claudia
              <br />
              <span className="text-primary">Fioravanti</span>
            </h1>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              <div className="mb-8">
                <div className="relative w-56 h-56 rounded-2xl overflow-hidden border-4 border-white shadow-card">
                  <Image src={profileImageSrc} alt={settings?.profileImage?.alt ?? doctorName} fill className="object-cover" />
                </div>
              </div>
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {credentials.map((abbr: string) => (
                    <div key={abbr} className="flex items-start gap-3 pb-4 border-b border-linen last:border-0 last:pb-0">
                      ...
                      <span className="font-body text-sm text-brand-muted leading-snug">{credentialFullNames[abbr] ?? abbr}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-cream-dark rounded-xl p-6 border border-linen">
                <p className="font-body text-sm text-brand-muted leading-relaxed italic">&ldquo;{quote}&rdquo;</p>
            ...
            {values.map((value: any) => (
              <div key={value.title} className="card p-6">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h3 className="font-heading text-lg font-semibold text-brand-text mb-1">{value.title}</h3>
                    <p className="font-body text-sm text-brand-muted leading-relaxed">{value.desc}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="section-pad bg-white">
        <div className="container-site text-center">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <Link href="/book" className="btn-primary px-8 py-3.5">
              Book a consultation <ArrowRight size={16} />
            </Link>
