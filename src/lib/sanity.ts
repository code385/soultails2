...
export async function getAllServices() {
  return sanityClient.fetch(`*[_type=="service"]|order(order asc){_id,name,"slug":slug.current,shortDescription,icon,serviceMode,priceDisplay,duration,featured}`)
}
export async function getServiceBySlug(slug: string) {
  return sanityClient.fetch(`*[_type=="service"&&slug.current==$slug][0]{_id,name,"slug":slug.current,shortDescription,fullDescription,icon,serviceMode,priceDisplay,duration,whatToExpect,metaDescription}`, { slug })
}
export async function getFeaturedServices() {
  return sanityClient.fetch(`*[_type=="service"&&featured==true]|order(order asc){_id,name,"slug":slug.current,shortDescription,icon,serviceMode,priceDisplay}`)
}
