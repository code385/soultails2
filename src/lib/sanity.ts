...
export async function getAllServices() {
  return sanityClient.fetch(`*[_type=="service"]|order(order asc){_id,name,"slug":slug.current,shortDescription,icon,image,serviceMode,priceDisplay,remotePrice,homeVisitPrice,duration,featured,bestFor,availability,pricingNote,ctaLabel,serviceAreaLabel,hidePublicPricing}`)
}
...
export async function getFeaturedServices() {
  return sanityClient.fetch(`*[_type=="service"&&featured==true]|order(order asc){_id,name,"slug":slug.current,shortDescription,icon,image,serviceMode,priceDisplay,remotePrice,homeVisitPrice,bestFor,availability,pricingNote,ctaLabel,serviceAreaLabel,hidePublicPricing}`)
}
